...
def test_spending_by_category_success(sample_transactions):
    with patch("pandas.read_excel", return_value=sample_transactions):
        result = spending_by_category(
            transactions=sample_transactions, category="Супермаркеты", start_date="2023-01-01", end_date="2023-01-31"
        )
...
def test_spending_by_category_empty_result(sample_transactions):
    result = spending_by_category(
        transactions=sample_transactions, category="Транспорт", start_date="2023-01-01", end_date="2023-01-31"
    )
...
def test_spending_by_category_date_filtering(sample_transactions):
    result = spending_by_category(
        transactions=sample_transactions, category="Супермаркеты", start_date="2023-01-01", end_date="2023-02-28"
    )
...
def test_spending_by_category_invalid_date():
    with pytest.raises(ValueError), patch("logging.error") as mocked_logging:
        spending_by_category(
            transactions=pd.DataFrame(), category="Тест", start_date="invalid-date", end_date="2023-01-01"
        )
